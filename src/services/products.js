...
const createSectionsForProducts = (quantityProductsTotal, quantityForProcess) => {
  const iterationesCicle = Math.ceil(quantityProductsTotal / quantityForProcess);
...
const getAllProducts = async (quantity, page) => {
  const url =
    "rest/all/V1/products?searchCriteria[currentPage]=" + page + "&searchCriteria[pageSize]=" + quantity + "&fields=items[id,name,sku,price,status,visibility,weight,extension_attributes,media_gallery_entries,product_links,custom_attributes]"
...
const getProductsForSku = async (listSku) => {
  ...
  const url =
    "rest/all/V1/products?searchCriteria[filter_groups][0][filters][0][field]=sku&searchCriteria[filter_groups][0][filters][0][value]=" + stringSKU + "&searchCriteria[filter_groups][0][filters][0][condition_type]=in&fields=items[id,name,sku,price,status,visibility,weight,extension_attributes,media_gallery_entries,product_links,custom_attributes]"
  return (await httpGet(url)).items;
}
  ...
  try {
    if (product.extension_attributes) {
      for (const webSiteId of product.extension_attributes.website_ids) {
        let store = storesCode.filter((storeCode) => storeCode.id == webSiteId)[0];
        store.stock = 0;
    ...
    console.log('se produjo un error en agregar el stock producto', e.response?.status)
  }
  return stores;
}
...
export const updateProducts = async (update) => {
  ...
  for await (const productToUpdate of update) {
    let product = JSON.parse(JSON.stringify((await getProductforSku(productToUpdate.sku))));
    if (product) {
      const upd = await Product.findOneAndUpdate({ sku: productToUpdate.sku }, { $set: { stores: productToUpdate.stores } }, { new: true });
      response.push(upd)
    }
  }
  return response;
...
export const updateProductsForSku = async (listSku) => {
  ...
  for (const product of products) {
    ...
    const response = await Product.updateOne({ sku: product.sku }, { ...product })
  }
}
...
export const getProductsBioInsuperables = async () => {
  return await Product.find({ stores: { $all: [{ "$elemMatch": { bioinsuperable: true } }] } });
}
